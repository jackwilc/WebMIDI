...
if (process.argv.length < 3) {
  ...
  var midiReceived = function(deltaTime, message) {
    ...
    if (message != 248) {
      ...
      if (message[1] == 18) {
        console.log('flux b on');
        io.sockets.emit('pitchona', message[2]);
      }
      if (message[1] == 19) {
        console.log('flux b on');
        io.sockets.emit('pitchonb', message[2]);
      }
...
io.sockets.on('connection', function(socket) {
  ...
  socket.on('notedown2', function(data) {
    midiOut.sendMessage(help.noteOn(62 + (10 * (fx - 1)), data.message1));
    //console.log('Midi out CH' + (60 + (10 * (fx - 1))) + "FX = " + fx);
  });
